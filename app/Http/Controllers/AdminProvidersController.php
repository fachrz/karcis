...
{
    public function pesawatairlines(){
        $airlines = pesawat_airlines_model::all();
        return view('admin.pesawat.PesawatAirlinesPage', [
            'airlines' => $airlines
        ]);
    }

    public function airlinesadd(Request $request){
    ...
    public function airlinesdelete($id_airlines){
        $airlines = pesawat_airlines_model::where('airline_id', $id_airlines);
        if ($airlines->delete()) {
